<template>
  <CommandPlane>
    <template #title>
      <span>{{ $translate("系统充放电有功功率设置") }}</span>
      <div>
        <el-button
          plain
          size="mini"
...
export default {
  ...
  methods: {
    ...
    issueCommand() {
      ...
      if (
        isNaN(Number(requestData.pcsActivePower)) ||
        requestData.pcsActivePower > 250 ||
        requestData.pcsActivePower < -250
      ) {
        return showMessage(
          "warning",
          this.$translate("请设置范围在") + " [-250,250]"
        );
      }
      this.$emit("issueCommand", {
        requestData: {
