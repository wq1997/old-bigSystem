    ...
    methods: {
        ...
        async getChartsData() {
            ...
            let { nowDatas, compareDatas, nowMaxMinAvg, compareMaxMinAvg } =
                data;
            this.chartArgs.options.series[0].data = nowDatas
                ? getEchatsData(nowDatas)
                : [];
            this.chartArgs.options.series[1].data = compareDatas
                ? getEchatsData(
                      nowDatas
                          ? makeSameDate(compareDatas, nowDatas)
                          : compareDatas
                  )
                : [];
            ...
            if (seriesData.length && seriesData[0].data) {
                ...
                seriesData[0].data.forEach((item, index) => {
                    ...
                    seriesData.forEach(({ name, data }, _uuIndex) => {
                        ...
                        dataItem[_uuIndex === 0 ? "今日" : "对比日期"] = value;
                        dataItem = {
                            ...dataItem,
                        };
                    });
